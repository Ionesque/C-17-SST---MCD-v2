...
public class AvionicsFaultPage : MonoBehaviour {
    ...
    public void SetAllFaults()
    {
        ResetFaults();

        // Perform a coin flip to see if values should be set
        globalAccess.misc_EFCSreset = false;
        globalAccess.misc_IRUaligned = false;
        globalAccess.misc_Rad1_On = false;
        globalAccess.misc_Rad2_On = false;
        globalAccess.misc_HF1_On = false;
        globalAccess.misc_HF2_On = false;
        
        globalAccess.fault_TAWS = true;
        globalAccess.fault_CCU1 = true;
        globalAccess.fault_OBIGGS = true;
        globalAccess.fault_BATT = true;

        globalAccess.stat_DBmismatch = true;
        globalAccess.stat_ChMast = true;
        globalAccess.stat_datalink = true;
    }
    ...
    void SetGlobalAccess()
    {
        try
        {
            globalObject = GameObject.Find("Fault Settings");
            globalAccess = globalObject.GetComponent<systemManager>();
            isReady = true;
